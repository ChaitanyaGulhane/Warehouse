...
*
* TCP Server
* This server helps to connect the client to required client and relay between them.
* This is an example of switching between fds with the help of epoll.
*
*/
...
#define log_err(arg, varg...) fprintf(stderr, "\n" arg, ##varg)
...
static int
cmd_help(struct _client *head, struct _client *p, char *buffer) {
...
struct _callback {
	int (*cmd) (struct _client *head, struct _client *p, char* buffer);
	char *command;	
};

static struct _callback callback [] = {
	{cmd_help, "HELP"},
	{cmd_ident, "IDENT"},
	{cmd_connect, "CONNECT"},
...
cmd_commands(struct _client **head, struct _client *p,  char * buffer) {
	...
	int i = 0;
	for(; i < 3; i++) {
		if(strncasecmp((buffer + 4), callback[i].command, strlen(callback[i].command)) == 0) {
			callback[i].cmd((*head), p, buffer);		
			flag = 1;
			break;
		}
	}
	if(!flag) {
		if(strcasecmp((buffer + 4), "CLOSE") == 0) {
	...
	while(!exit_global) {
		...
		for(i = 0; i < nfd; i++) {
			if(events[i].data.ptr == NULL) {
				...
				if (client_connect(sockfd, epollfd, &head) < 0) {
					goto cleanup;
				}
			}
			else {
				p = (struct _client *) events[i].data.ptr;
				tempfd = p->fd;
				...
				if((len = send(tempfd, buffer, strlen(buffer), 0)) <  0) {
					...
					log_err("send(%d) error => %s", tempfd, strerror(errno));
				}
			}
		}
	}
